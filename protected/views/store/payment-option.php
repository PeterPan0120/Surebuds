...
<div class="sections section-grey2 section-payment-option">
   <div class="container">
     ...
     <?php else :?> <!-- DELIVERY-->                          	       	      
		  ...
		  <p>
	        <?php echo clearString(ucwords($merchant_info['restaurant_name']))?> <?php echo Yii::t("default", "")?> 
           ...
           <?php if (!$search_by_location):?>    
           <?php
            if(!($cities=Yii::app()->functions->getCityList()))
                $cities = null;
            $cityList = array();
            if($cities!=null){
                foreach($cities as $key=>$city){
                    $k = strval($city['id'])."_".strval($city['delivery_fee']);
                    $cityList[$k]=$city['city'];
                }
            }
            else $cityList = null;
        	?>
              <div class="row top10">
                <div class="col-md-10">
	              <?php echo CHtml::dropDownList('city',
            ...
            <div class="row top10">
                <div class="col-md-10">
                 ...
                 ,array(
                 'class'=>'grey-fields full-width',
	               'placeholder'=>Yii::t("default","Province"),
	               'data-validation'=>"required"
	              ))?>
	             </div> 
              </div>  
             ...
             <div class="row top10">
                <div class="col-md-10">
                  ...
                  ,array(
	               'class'=>'grey-fields full-width',
	               'placeholder'=>Yii::t("default","Postal code"),
	               'data-validation'=>"required"
	              ))?>
	             </div> 
              </div> 
           ...
           <?php else :?>      
            ...
            <?php             
             ...
             if(is_array($location_search_data) && count($location_search_data)>=1){
             	$citys=FunctionsV3::ListCityList($location_search_data['state_id']);
             	$areas=FunctionsV3::AreaList($location_search_data['city_id']);
             } else {
             	$citys=array(
             	  ''=>t("Select City")
             	);
