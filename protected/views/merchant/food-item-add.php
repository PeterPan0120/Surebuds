...
if (isset($_GET['id'])){
	...
	$count=isset($data['count'])?(array)json_decode($data['count']):false;	
...
<div class="uk-grid">
	...
	<?php echo CHtml::numberField('quantity',
...
<!--GALLERY -->
	...
	<li>
	  <div class="uk-grid">
	    ...
	    <?php else :?> 
	    <div class="uk-width-1-4"><?php echo Yii::t("default","Size")?></div>
	    <div class="uk-width-1-4"><?php echo Yii::t("default","Price")?></div>
	    <div class="uk-width-1-4"><?php echo Yii::t("default","Count")?></div>
	    <?php endif;?>
	  </div>
	</li>	
	...
	<?php if ( is_array($price) && count($price)>=1):?>
		...
		<?php foreach ($price as $price_key=>$val_price):
		    $val_count=0;
		    foreach($count as $count_key=>$val_cnt)
		        if($price_key==$count_key)
		            $val_count = $val_cnt;
		?>			
		<li class="<?php echo $x==count($price)?"price_wrap":"";?>">
		  <div class="uk-grid">
		    ...
		    <div class="uk-width-1-4">
		      <?php echo CHtml::textField('count[]',$val_count,
		      array('class'=>'uk-form-width-medium numeric_only'))?>
		    </div>		
		    <div class="uk-width-1-4">
		    <a href="javascript:;" class="removeprice"><i class="fa fa-minus-square"></i></a>
