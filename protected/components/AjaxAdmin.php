...
if (!class_exists('AjaxAdmin'))
{
	class AjaxAdmin extends DbExt
	{
	    ...
	    {	    	
	    	...
	    	$count='';
	    	if (isset($this->data['count']) && count($this->data['count'])>=1){
	    		foreach ($this->data['count'] as $key=>$val) {
	    			if (!empty($val)){
	    			   $count[$this->data['size'][$key]]=$val;
	    			}
	    		}	    		
	    	}	  
	    	...
	    	$params=array(			  
			  ...
			  'count'=>isset($count)?json_encode($count):'',
	    ...
	    {	  
    	    ...
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {  
    	    	    ...
    	    	    if(isset($_SESSION['kr_merchant_user'])){
                        if(json_decode($_SESSION['kr_merchant_user'])[0]->user_access){
                            $user_access = json_decode($_SESSION['kr_merchant_user'])[0]->user_access;
                            
                            if(strpos($user_access, 'EditInventory') !== false)
                                $option="<div class=\"options\">
                    	    		<a href=\"$slug/id/$val[item_id]\" >".Yii::t("default","Edit")."</a>
                    	    		</div>";
                    	    		
                            else{
                                $merchant_user_id = json_decode($_SESSION['kr_merchant_user'])[0]->merchant_user_id;
                                $merchant_user = Yii::app()->functions->getMerchantUserInfo($merchant_user_id);
                                if(strpos($merchant_user['user_access'], 'EditInventory')!==false)
                	    			$option="<div class=\"options\">
                        	    		<a href=\"$slug/id/$val[item_id]\" >".Yii::t("default","Edit")."</a>
                        	    		</div>";
                            }
    ...
    if (is_numeric($mt_delivery_miles)){    	
	    ...
	    public function addShippingRate(){
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])){				
				unset($params['date_created']);
				$params['date_modified']=FunctionsV3::dateNow();				
				$res = $command->update('{{delivery_rate}}' , $params , 
				'id=:id' , array(':id'=>addslashes($this->data['id'])));
				if ($res){
					$this->code=1;
	                $this->msg=Yii::t("default",'Delivery Fee. updated.');  
				} else $this->msg=Yii::t("default","ERROR: cannot update");
			} else {				
				if ($res=$command->insert('{{delivery_rate}}',$params)){
					$this->details=Yii::app()->db->getLastInsertID();	
	                $this->code=1;
	                $this->msg=Yii::t("default",'Delivery Fee added.');  	                
	            } else $this->msg=Yii::t("default",'ERROR. cannot insert data.');
			}	    		    	
	    }
	    ...
	    {	    	
	    	...
	    	if ( !$res=Yii::app()->functions->isClientExist($this->data['email_address']) ){
	    		...
	    		$params=array(
	    		  ...
	    		  'city'=>$p->purify(Yii::app()->functions->getCityFromId(explode("_", $this->data['city'])[0])[0]['city']),
...
//dump($params); die();
	    				...
	    				if ( $this->insertData("{{order}}",$params)){
		    			   ...
		    			   /*VOUCHER*/
		    				...
		    				foreach ($raw['item'] as $val) {		    					
				                ...
				                $updated_params['date_modified']=FunctionsV3::dateNow();
				                $selected_food_item = Yii::app()->functions->getFoodItem2($val['item_id']);
				                $f_items = json_decode($selected_food_item['count']);
				                $deducted_count = 0;
				                foreach($f_items as $key=>$f_item)
				                    if(Yii::app()->functions->getFoodSize($key)['size_name']==$val['size_words']){
				                        
				                        $deducted_count = $f_item;
				                    }
				                $updated_params['quantity'] = $quantity-($deducted_count)*$val['qty'];
				                $command = Yii::app()->db->createCommand();
		    					$res = $command->update('{{item}}' , $updated_params , 
				 'item_id=:item_id' , array(':item_id'=>addslashes($val['item_id'])));
		    				}
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		foreach ($res as $val) {	
                    ...
                    if(isset($_SESSION['kr_merchant_user'])){
                        if(json_decode($_SESSION['kr_merchant_user'])[0]->user_access){
                            $user_access = json_decode($_SESSION['kr_merchant_user'])[0]->user_access;
                            if(strpos($user_access, 'RemovablePermission') !== false)
                                $action.="<a href=\"javascript:;\" class=\"row_del\" rev=\"".$val['order_id']."\" >".Yii::t("default","Delete")."</a>";
                        }
                        else
        	    			$action.="<a href=\"javascript:;\" class=\"row_del\" rev=\"".$val['order_id']."\" >".Yii::t("default","Delete")."</a>";
                    }
				    // $action.="<a data-id=\"".$val['order_id']."\" class=\"order-delete\" href=\"javascript:\">".Yii::t("default","Delete")."</a>";
        ...
        public function chartOriginalSales()
	    {
	    	$merchant_id=Yii::app()->functions->getMerchantID();	    
	    	...
	    	$date_now=date('Y-m-d 23:00:59');
	    	$start_date=date('Y-m-d 00:00:00',strtotime($date_now . "-30 days"));
	    	$stmt="SELECT DATE_FORMAT(date_created, '%M-%D') as date_created_format,SUM(sub_total) as total
	    	FROM
	    	{{order}}
	    	WHERE
	    	date_created BETWEEN ".FunctionsV3::q($start_date)." AND ".FunctionsV3::q($date_now)."
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
		...
		public function merchantCustomerList()
		{
			...
			if ( $res=$this->rst($stmt)){		
				...
				foreach ($res as $val) {	
					...
					//$slug/Do/Add/?id=$val[client_id]
					$link=Yii::app()->createUrl('merchant/customerlist/Do/Add',array(
					 'id'=>$val['client_id']
					));
					$viewlink=Yii::app()->createUrl('merchant/getOrderList/',array(
					 'id'=>$val['client_id']
					));
					
					$action="<div class=\"options\">
    	    		<a href=\"$link\" >".Yii::t("default","Edit")."</a>
    	    		<a href=\"$viewlink\" >".Yii::t("default","View Orders")."</a>
    	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[client_id]\" >".Yii::t("default","Delete")."</a>
    	    		</div>";	
		...
		public function merchantGuestList()
		{
