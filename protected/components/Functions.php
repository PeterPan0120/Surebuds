...
{	
    ...
    public function jsLanguageAdmin()
    {
    	...
    	return array(
    	  ...
    	  "lastOriginalSales"=>Yii::t("default","Last 30 days Original Sales With No Delivery Fee"),
	...
	public function merchantMenu()
	{
		...
		$payment_list=array('visible'=>$this->hasMerchantAccess("payment-gateway"),'tag'=>'payment-gateway','label'=>'<i class="fa fa-usd"></i>'.Yii::t("default",'Payment Gateway'),
                   ...
                   /*array('visible'=>$this->hasMerchantAccess("psk"),'tag'=>'psk', 'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Paystack"), 
                   'url'=>array('merchant/paystack')),                                                                                                 */
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('visible'=>$this->hasMerchantAccess('customerlist'),'tag'=>"customerlist",
                'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Customer List"),
                'url'=>array('/merchant/customerlist')),
                
                
                array('visible'=>$this->hasMerchantAccess('guestlist'),'tag'=>"guestlist",
                'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Guest List"),
                'url'=>array('/merchant/guestlist')),
                
                array('visible'=>$this->hasMerchantAccess('inventory'),'tag'=>"inventory",
                'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Inventory"),
                'url'=>array('/merchant/inventory')),
                ...
                array('visible'=>$this->hasMerchantAccess("reports"),'tag'=>'reports','label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default",'Reports'),
                   ...
                   array('visible'=>$this->hasMerchantAccess("chartreport"),'tag'=>'chartreport','label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Report With Chart"), 
                   'url'=>array('merchant/chartreport')),      
                )),
                ...
                array('visible'=>$togle_com,'tag'=>"RemovablePermission",'label'=>'<i class="fa fa-users"></i>'.Yii::t("default","Removable Permission"),
                'url'=>array('/merchant/RemovablePermission')),
                
                array('visible'=>$togle_com,'tag'=>"EditInventory",'label'=>'<i class="fa fa-users"></i>'.Yii::t("default","Edit Inventory"),
                'url'=>array('/merchant/EditInventory')),
    ...
    public function getFoodSize($size_id='')
    {
    	$merchant_id=$this->getMerchantID();
    	$DbExt=new DbExt;
	    $stmt="SELECT * FROM
    ...
    public function getCityFromId($id){
        $lists='';
		$DbExt=new DbExt;
		$stmt="SELECT city FROM {{delivery_rate}} WHERE id=".$id;
		if ( $res=$DbExt->rst($stmt)){			
			return $res;
		}
		return false;
	...
	{		
		$lists='';
		$DbExt=new DbExt;
		$stmt="SELECT id, delivery_fee, city FROM {{delivery_rate}} where merchant_id=".$_SESSION['kr_merchant_id']." ORDER BY city ASC";
		if ( $res=$DbExt->rst($stmt)){			
			return $res;
		}
		return false;
	...
	public function getItemSize($item_id){}
...
'sub_item_name',$addon_name_trans).'</div>';
    			...
    			else if(!isset($data['delivery_charge']))   	{
    			    if(isset($_SESSION['delivery_fee']))
		                $total=$subtotal+$taxable_subtotal+$_SESSION['delivery_fee']+$merchant_packaging_charge;
		            else
		                $total="undefined";
    			}
		        else	 
		            $total=$subtotal+$taxable_subtotal+$data['delivery_charge']+$merchant_packaging_charge;
		        ...
		        $htm.='<div class="summary-wrap">';
		         ...
		         if ( $free_delivery==true){
		             $htm.=FunctionsV3::receiptRowTotal("Estimated Delivery Fee", t("Free") );
		             $_SESSION['delivery_fee'] = 0;
		         }
		         else{ 
		             if (!isset($data['delivery_charge'])){
    		             if(isset($_SESSION['delivery_fee'])){
        		             $htm.=FunctionsV3::receiptRowTotal('Estimated Delivery Fee', t("$".$_SESSION['delivery_fee']));
        		         }
        		         else{
        		             $htm.=FunctionsV3::receiptRowTotal('Estimated Delivery Fee', t("$15 - $30"));
        		         }
    		         }	
    		         else $htm.=FunctionsV3::receiptRowTotal('Estimated Delivery Fee', t("$".number_format($data['delivery_charge'], 2)));
		         }
		         
		         //echo $subtotal."   +   ".$_SESSION['delivery_fee'];
	    	  ...
	    	  // else $htm.="<div class=\"col-md-6 col-xs-6  text-right cart_total\">Undefined</div>";
	    	   $htm.="</div>";
		       ...
		       $item_array_total=array(
		         ...
		         'delivery_charges'=>isset($_SESSION['delivery_fee'])?$_SESSION['delivery_fee']:$delivery_charges,
		         'total'=>$total,		         
		         'tax'=>$tax,
		         'tax_amt'=>$tax_amt,
		         'curr'=>baseCurrency(),
    ...
    public function isClientLogin()
    {
    	if (isset($_SESSION['kr_client'])){
    		if (array_key_exists('client_id',$_SESSION['kr_client'])){    			
    			if (is_numeric($_SESSION['kr_client']['client_id']) && $_SESSION['kr_client']['is_guest']==2){
    				return true;
    			}
    		}    	
    	}
	...
	public function getOrder2($order_id='')
	{
		...
		(
		select concat(street,' ',area_name,' ',state,' ',zipcode )
		from
		{{order_delivery_address}}
		where
		order_id=a.order_id
    ...
    public function sendSMS($to='',$message='' , $record_id='')
    {
    	...
    	switch ($sms_provider) {
    		...
    		case "infobip":
    			...
    			try {
    				$raw = infobip::sendSMS($info_params);
    				$msg ="process";
    			} catch (Exception $e){
    				$msg  = $e->getMessage();
    			}
